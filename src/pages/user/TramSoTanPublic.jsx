...
import { fetchShelters } from "../../features/shelters/shelters";

export default function TramSoTanPublic() {
  const dispatch = useDispatch();
  const { items, loading, error } = useSelector(s=>s.shelters);
  const [kw, setKw] = useState("");

  useEffect(()=>{ dispatch(fetchShelters()); }, [dispatch]);

  const filtered = useMemo(()=>{
    const k = kw.toLowerCase();
    return (items||[]).filter(x =>
      (x.ten_khu_vuc||"").toLowerCase().includes(k) ||
      (x.dia_chi||"").toLowerCase().includes(k)
    );
  }, [items, kw]);
