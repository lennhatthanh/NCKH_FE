...
export default function Home() {
    ...
    <div className="space-y-14">
      <section className="relative rounded-3xl overflow-hidden">
        ...
        <div className="relative z-10 px-6 py-16 md:py-28 max-w-5xl mx-auto text-white">
          ...
          <p className="text-lg max-w-xl mb-6 opacity-90">
            Kết nối nhanh chóng giữa người cần giúp đỡ và lực lượng hỗ trợ. Cập
      ...
      <section className="max-w-6xl mx-auto px-4">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {latest3.length > 0
            ? latest3.map((tb) => <AnnouncementCard key={tb.id} item={tb} />)
            : [1, 2, 3].map((i) => (
      ...
      <section className="max-w-6xl mx-auto px-4">
  <h2 className="text-2xl font-semibold mb-4">Hình ảnh hoạt động</h2>
  <div className="columns-2 md:columns-4 gap-4 space-y-4">
    {[
      ...
      const targetUrl = "https://i.pinimg.com/736x/45/04/b2/4504b22b17c2bcd1462351e96d03ab29.jpg";

      if (url === targetUrl) {
        return (
          <div key={idx} className="rounded-xl border border-ocean-100 w-full relative overflow-hidden" style={{ paddingBottom: '150%' }}>
            <img
              src={url}
              alt={`gallery-${idx}`}
        ...
        <div className="rounded-3xl overflow-hidden bg-gradient-to-r from-ocean-500 to-ocean-300 text-white p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="opacity-90">
              Gửi yêu cầu kèm thông tin chi tiết để chúng tôi hỗ trợ nhanh nhất
              có thể.
            </p>
          </div>
          <NavLink
...
function AnnouncementCard({ item }) {
  return (
    <div className="rounded-2xl border border-ocean-100 bg-white p-5 flex flex-col">
      ...
      <p className="text-sm text-ocean-700 mt-1 line-clamp-3">{item.noi_dung}</p>
    </div>
  );
}
